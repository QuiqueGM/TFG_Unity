Shader "socialPointCG/sCG_Fx_sea2_TRANSPARENT" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			half _ampX;
			half _ampY;
			half  	_ReflectionBend;
			half _Overbright;
			half  	_ReflectionAmount;
			half 		_DistorsionAmount;
			half		_CausticsAmount;
            CBUFFER_END
            ...
            float4 ObjectToClipPosAlt(float3 pos)
            {
                VertexPositionInputs vertInputs = GetVertexPositionInputs(pos.xyz);    //This function calculates all the relative spaces of the objects vertices
                return vertInputs.positionCS;
            }
			...
			float4 frag(vertexOutput i) : COLOR
			{
				...
				float4 DF = float4(0,0,0,i.DFCol.a);

				float4 NM1 = tex2D(_NM, i.uvNM_A.xy);
				float4 NM2 = tex2D(_NM, i.uvNM_B.xy);
				float4 NM = NM1 + NM2 * 0.5;
				NM = NM * 2.0 - 1.0;
				NM.g *= -1.0;

				float3 viewDir = normalize(i.vDir);

				float NdotV = max(0.0, dot(viewDir, normalize(NM.rgb)));
				NdotV = NdotV * NdotV;
				NdotV = 1.0 - NdotV;
				...
				float4 CA1 = tex2D(_CA, i.uvNM_A.zw);
				float4 CA2 = tex2D(_CA, i.uvNM_B.zw);
				float4 CA = CA1 + CA2 * 0.5;
				...
				float4 NDIS = tex2D(_NDIS, refUV);
				//fixed NDIS_hi = smoothstep(0.75, 1.0, NDIS.r);
				NDIS = NDIS * _ReflectionAmount * DF.a;

				float4 Caustics = CA * (1.0 - DF.a) * _CausticsAmount;

				float4 Complete = i.DFCol.r * (DF + NDIS + Caustics);

				//Complete = mix(_SSSColor, Complete, NdotV);

				return float4(Complete.rgb, 0.0);
